import React from "react";

const SideBar = () => {
    return <React.Fragment>
        <div className="h-full">
            <ul className="h-full  menu bg-base-200 rounded-box bg-transparent p-0">
                <li>
                    <a className="tooltip tooltip-right " data-tip="Home">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                    </a>
                </li>
                <div className="divider divider-vertical m-0 mt-0 p-0"></div>

                <li>
                    <a className="tooltip tooltip-right " data-tip="Details">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </a>
                </li>
                <div className="divider divider-vertical m-0  p-0"></div>

                <li>
                    <a className="tooltip tooltip-right " data-tip="Stats">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
                    </a>
                </li>
            </ul>
        </div>
        <div className="divider divider-horizontal m-0  p-0 w-1"></div>
        <div h-full>
            <div className="h-9 flex items-center pl-3"><a>personal-info</a></div>
            <div className="w-full divider divider-vertical m-0 mt-0 p-0"></div>
            <ul className="h-full menu bg-base-200 w-56 rounded-box bg-transparent p-0">
                <li className=""><a>bio</a></li>
                <li className=""><a>interests</a></li>
                <li className="">
                    <details >
                        <summary>education</summary>
                        <ul>
                            <li className=""><a>high-school</a></li>
                            <li className=""><a>university</a></li>
                        </ul>
                    </details>
                </li>
                <div className="divider divider-vertical m-0 mt-0 p-0"></div>
                <li className="">
                    <details >
                        <summary>contacts</summary>
                        <ul>
                            <li className=""><a>user@gmail.com</a></li>
                            <li className=""><a>+3598246359</a></li>
                        </ul>
                    </details>
                </li>
            </ul>
        </div>
        <div className="divider divider-horizontal m-0  p-0 w-1"></div>
    </React.Fragment>
}
export default SideBar